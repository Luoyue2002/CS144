...
class NetworkInterface {
  private:
    ...
    ///! ARP table   size_t 过期时间
    //       map<ip pair<Address, time >> 30s
    std::map<uint32_t, std::pair< EthernetAddress, size_t>> _arp_table{};
    size_t  _arp_default_ttl = 30000 ;

    /// waiting arp response
    //map<ip ,time> 5s
    std::map<uint32_t, size_t> _ip_waiting_arp{};
    size_t  _ip_waiting_default_ttl = 5000 ;

    std::vector<std::pair<Address, InternetDatagram>> _internet_datagram_waiting_arp{};
